...
if ($_REQUEST['act'] == 'index')
{
    ...
    // 判断活动是否存在
    $active_num = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('favourable_activity')." WHERE act_id = '$active_id' ");
    if ($active_num == 0)
    {
        show_message('活动不存在');
    }
...
//            print_arr(cart_favourable_goods($goods->active_id));
...
function favourable_goods_list($user_rank, $favourable_id, $sort='', $order='', $size, $page, $warehouse_id = 0, $area_id = 0)
{
    ...
    $sql = "SELECT act_range_ext, act_range, userFav_type, user_id " .
            ...
            " AND start_time <= '$now' AND end_time >= '$now' AND act_id = '$favourable_id' ";
...
{
    $favourable = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('favourable_activity') . " WHERE act_id = '$favourable_id' ");
...
function get_act_type($user_rank, $favourable_id)
{
    ...
    $sql = "SELECT act_type " .
            ...
            " WHERE CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%'" .
            " AND start_time <= '$now' AND end_time >= '$now' AND act_id = '$favourable_id' ";
    $act_type = $GLOBALS['db']->getOne($sql);
    $act_type_txt = '';
    switch ($act_type)
    {
        case 0:
