...
if ($_REQUEST['act'] == 'gengduo_tid')
{
    ...
    $sql = "SELECT `id`,`cat_id`,`title`,`start_time`,`end_time`,`amount`,`join_money`,`join_num`,`title_img`,`describe`,(end_time-unix_timestamp(now())) as shenyu_time FROM".$ecs->table('zc_project')."WHERE tid='$tid' $where_wenzi ORDER BY id desc";
    $zc_arr = $db->getAll($sql);
...
if($_REQUEST['act'] == 'detail')
{
	...
	if(empty($zhongchou))
	{
		$sql = " SELECT id FROM ".$ecs->table('zc_project')." limit 1 ";
		$first_id = $db->getOne($sql);
	...
	{
		$sql=" select rec_id from ".$GLOBALS['ecs']->table('zc_focus')." where pid= ".$cid." and user_id= ".$_SESSION['user_id'];
    ...
    foreach($zc_evolve_list as $k=>&$v){
        $v['pro-day']=floor((time()-$v['add_time'])/86400);
	...
	if($_SESSION['user_id']>0)
	{
		if(!empty($topic_content))
		{
			$sql=" insert into ".$GLOBALS['ecs']->table('zc_topic').
				...
				" VALUES ".
				" (NULL, '".$topic_id."', '".$parent_id."', 1, '".$topic_content."', '".$_SESSION['user_id']."', '".$zcid."', '".time()."') ";
	...
	if($_SESSION['user_id']>0)
	{
		if(!empty($topic_content))
		{
			$sql=" insert into ".$GLOBALS['ecs']->table('zc_topic').
				...
				" VALUES ".
				" (NULL, 0, 1, '".$topic_content."', '".$_SESSION['user_id']."', '".$zcid."', '".time()."') ";
			if($GLOBALS['db']->query($sql))
			{
				$result['error']=1;
				$result['message']='发布成功';
...
if ($_REQUEST['act'] == 'checkout')
{
    ...
    // {
		// show_message('收货信息不完整，请将必填信息填写完整后再重新提交', $_LANG['back_up_page'], 'crowdfunding.php?act=consignee&gid='.$_REQUEST['gid']);
    // }
	...
	if($shengyu == 0)
	{
		show_message('该产品已售罄，请选择其他产品', $_LANG['back_up_page'], 'javascript:history.back(-1)');
	}
...
if ($_REQUEST['act'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
		...
		$smarty->assign('page_title', '众筹-收货地址'); //页面标题 by wu
    ...
    else
    {
        ...
        Header("Location:crowdfunding.php?act=checkout&gid=$_POST[gid]");
        exit;
    }
}
...
if ($_REQUEST['act'] == 'drop_consignee')
{
    ...
    {
        ecs_header("Location: crowdfunding.php?act=consignee&gid=".$_REQUEST['gid']."\n");
        exit;
    }
    else
    {
		...
		if($type==1)
		{
			...
			else
			{
				...
				if(empty($focus_status))
				{
					...
					" VALUES ".
					" (NULL, '".$_SESSION['user_id']."', '".$zcid."', '".time()."') ";
					if($GLOBALS['db']->query($sql))
					{
...
if ($_REQUEST['act'] == 'done')
{
	...
	{
		show_message('您没有设置收货地址','返回上一页','javascript:history.back(-1)');
	}
	...
	if($zc_order_num > 0)
	{
		show_message('您有未支付的众筹订单，请付款后再提交新订单','返回上一页','user.php?act=crowdfunding');
	}
    ...
    $order['country'] =  !empty($_POST['country']) ? trim($_POST['country']) : 1;
    $order['province'] = !empty($_POST['province']) ? trim($_POST['province']) : 0;
    $order['city'] = !empty($_POST['city']) ? trim($_POST['city']) : 0;
    $order['district'] = !empty($_POST['district']) ? trim($_POST['district']) : 0;
...
HTML;
		...
		<a href="flow.php?step=done&act=balance&order_sn={$order['order_sn']}" id="balance" style="float: left;" order_sn="{$order['order_sn']}" flag="balance" >余额支付</a>
HTML;
				}
...
HTML;
	...
	/* remove by wu 数据在支付成功后更新
    //增加支持的用户id
    $sql = "SELECT zhichi FROM".$ecs->table('zc_goods')."WHERE id = '$_POST[gid]'";
    $zc_num = $db->getOne($sql);
    $zc_num += 1;

    $sql = "UPDATE ".$ecs->table('zc_goods')."SET zhichi=$zc_num WHERE id = '$_POST[gid]'";
    $db->query($sql);

    $sql = "SELECT zhichi_str FROM".$ecs->table('zc_goods')."WHERE id = '$_POST[gid]'";
    $zhichi_str = $db->getOne($sql);
    if(empty($zhichi_str)){
        $zhichi_str = $_SESSION['user_id'];
    }else{
        $zhichi_str = $zhichi_str.','.$_SESSION['user_id'];
    }

    $sql = "UPDATE ".$ecs->table('zc_goods')."SET zhichi_str='$zhichi_str' WHERE id = '$_POST[gid]'";
    $db->query($sql);

    $sql = " SELECT join_num FROM ".$ecs->table('zc_project')."WHERE id = '$_POST[xm_id]'";
    $xm_zhichi = $db->getOne($sql);
    $xm_zhichi += 1;

    $sql = "UPDATE ".$ecs->table('zc_project')."SET join_num=$xm_zhichi WHERE id = '$_POST[xm_id]'";
    $db->query($sql);

    //增加众筹金额
    $sql = "SELECT join_money FROM".$ecs->table('zc_project')."WHERE id = '$_POST[xm_id]'";
    $get_money = $db->getOne($sql);
    $get_money += $order['goods_amount'];

    $sql = "UPDATE ".$ecs->table('zc_project')."SET join_money = $get_money WHERE id = '$_POST[xm_id]'";
    $db->query($sql);
	*/
...
if ($_REQUEST['act'] == 'rm_focus')
{
    ...
    if($res){
        show_message('取消关注成功!','返回上一页','user.php?act=crowdfunding');
    }
...
function setNumberFormat($number=0,$limit=0,$point=0,$round=true)
{
	...
	switch($count-1)
	{
		case 0 : $dividend=1; $name="个"; break;
		case 1 : $dividend=10; $name="十"; break;
		case 2 : $dividend=100; $name="百"; break;
		case 3 : $dividend=1000; $name="千"; break;
		case 4 : $dividend=10000; $name="万"; break;
		case 5 : $dividend=100000; $name="十万"; break;
		case 6 : $dividend=1000000; $name="百万"; break;
		case 7 : $dividend=10000000; $name="千万"; break;
		case 8 : $dividend=100000000; $name="亿"; break;
		default : $dividend=1; $name="个"; break;
	}
