...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('order_list.dwt'), '',
...
elseif ($_REQUEST['act'] == 'pick_goods')
{
	...
	if($pick_code)
	{
		$store_order_info = get_store_order_info($id);
		//判断订单是否已付款
		$sql = " SELECT order_status, pay_status, shipping_status FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id = '$order_id' ";
		$order_info = $GLOBALS['db']->getRow($sql);
		if($order_info['pay_status'] != 2)
		{
			$result['error'] = 0;
			$result['message'] = "订单未付款，无法提货";			
		}
		elseif($store_order_info && $store_order_info['pick_code'] == $pick_code)
		{
			$sql = " UPDATE ".$GLOBALS['ecs']->table('order_info')." SET order_status = '1', pay_status='2', shipping_status='1' WHERE order_id = '$order_id' "; 
			if($GLOBALS['db']->query($sql))
			{
				$result['error'] = 1;
				$result['message'] = "操作成功，提货完成";
				//sys_msg('操作成功，提货完成', 0, $link);
			}	
		}	
		else
		{
			...
			//sys_msg('请输入正确的提货码', 0, $link);
		}		
	}
	else
	{
		$result['error'] = 0;  
		$result['message'] = "请输入6位提货码";     
        //sys_msg('请输入6位提货码', 1, $link);
	}
	die(json_encode($result));
}
            ...
            if ($order_finish) {
                ...
                if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '') {
                    ...
                    if ($GLOBALS['_CFG']['sms_type'] == 0) {
                        ...
                        $smsParams = array(
                            'shop_name' => '',
                            'user_name' => $shop_name ? $shop_name : '',
                            'order_msg' => $msg ? $msg : '',
                            'mobile_phone' => $order['mobile'] ? $order['mobile'] : ''
                        );
...
//	$link[] = array('href' => 'order.php?act=list', 'text' => '门店订单列表');
//	
//	$sql = " UPDATE ".$GLOBALS['ecs']->table('order_info')." SET shipping_status='1',order_status = 5 WHERE order_id = '$order_id' "; 
//	if($GLOBALS['db']->query($sql))
//	{
//		//sys_msg('操作成功，发货完成', 0, $link);
//		make_json_response('', 1, '操作成功，发货完成', array('url'=>'order.php?act=list'));
//	}
//	else
//	{
//		//sys_msg('操作异常，请稍后再试', 0, $link);
//		make_json_response('', 0, '操作异常，请稍后再试', array('url'=>'order.php?act=list'));
//	}
}
...
function store_order_list($ru_id = 0, $store_id = 0)
{
	...
	/* 初始条件 */
	$limit = "";
	//$limit = " AND $store_id IN (SELECT so2.grab_store_list FROM ".$GLOBALS['ecs']->table('store_order')." AS so2 WHERE so2.id = so.id) ";
    $where = " WHERE 1 AND (so.store_id = '$store_id' OR (so.store_id = '0' AND so.is_grab_order = '1' ".$limit.")) ";
