...
//    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('collect_brand') . " WHERE user_id = '$user_id' ORDER BY rec_id DESC ". $limit;\
    $sql = "(SELECT cb.*, b.brand_name, b.brand_logo FROM " . $GLOBALS['ecs']->table('collect_brand') .
            " AS cb LEFT JOIN " . $GLOBALS['ecs']->table('brand') . " AS b ON cb.brand_id = b.brand_id WHERE cb.ru_id = 0 AND b.brand_id > 0)
            UNION
...
//            $brand_list[$row['rec_id']]['brand_info'] = get_brand_info($mbid, $mact);
            // 本人判断是否收藏， 已经该品牌收藏人数
        ...
        {
//            $brand_list[$row['rec_id']]['brand_info'] = get_brand_info($row['brand_id']);
        ...
        $brand_list[$row['rec_id']]['brand_goods']        = brand_get_goods($brand_id, $mbid, $cat, $size, $page, $sort, $order, $region_id, $area_info['region_id'], $act, $ship, $price_min, $price_max, $self);
        
    }
    
    $arr = array('brand_list' => $brand_list, 'record_count' => $record_count, 'paper' => $paper, 'size' => $size);
