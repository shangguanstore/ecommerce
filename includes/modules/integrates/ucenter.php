...
class ucenter extends integrate
{
    ...
    function add_user($username, $password, $email)
    {
        ...
        else
        {
            ...
            $ip = real_ip();
            $password = $this->compile_password(array('password'=>$password));
            $this->db->query('INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '" .$email['email']. "', '$username', '$password', '$reg_date', '$reg_date', '$ip')");
            return true;
        }
    }
    ...
    function set_cookie($username='',$email_phone = 0, $is_oath = 1)
    {
        ...
        else
        {
            if ($is_oath == 1) {
                ...
                $sql = "SELECT u.user_id, u.password, u.email, identifier FROM " . $GLOBALS['ecs']->table('users') . " AS u," .
