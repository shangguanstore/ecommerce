...
function assign_ur_here($cat = 0, $str = '', $strArr = array(), $url = '', $ru_id = 0)
{
    ...
    if (!in_array($filename, array('category', 'goods', 'single_sun','brand'))){
    ...
    {
        /* 处理有分类的 */
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand', 'single_sun', 'store_street')))
        {
            /* 商品分类或商品 */
            if ('category' == $filename || 'goods' == $filename || 'brand' == $filename || 'single_sun' == $filename)
            {
                if ($cat > 0)
                {
                    $cat_arr = get_parent_cats($cat);
                    $cat_arr = get_delete_store_cat($cat_arr);
    ...
    if (!empty($str))
    {
        ...
        $ur_here    .= '<span class="arrow">></span>'.' <span>'. $str. '</span>';
    }
 ...
 * 删除商家分类
 */
function get_delete_store_cat($cat_arr){
    ...
    while (1)
    {
        foreach ($arr AS $row)
        {
            if ($cat == $row['cat_id'])
            {
                ...
                $mcatInfo = $GLOBALS['db']->getRow('SELECT cat_id, user_id FROM ' . $GLOBALS['ecs']->table('merchants_category') . "WHERE cat_id = '" .$row['cat_id']. "' LIMIT 1");
                $cats[$index]['ru_id']   = isset($mcatInfo['user_id']) ? $mcatInfo['user_id'] : 0;
                
                $cats[$index]['cat_id']   = $row['cat_id'];
                $cats[$index]['cat_name'] = $row['cat_name'];
                $cats[$index]['parent_id'] = $row['parent_id'];
                
                $sql = "SELECT c.cat_id, c.cat_name FROM " .$GLOBALS['ecs']->table('category'). " AS c WHERE parent_id = '" .$row['cat_id']. "' " .
                        "AND (SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('merchants_category'). " AS mc WHERE mc.cat_id = c.cat_id LIMIT 1) = 0";
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='', $merchant_id = 0, $keyword = '', $ubrand = '', $act = '',$ship='', $self='', $mbid = 0)
{
    ...
    {
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'ubrand' => $ubrand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type, 'ship' => $ship, 'self' => $self);
            break;	
        ...
        case 'article_cat':
            ...
            break;
        case 'brand':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'mbid' => $mbid, 'price_min'=>$price_min, 'price_max'=>$price_max, 'sort' => $sort, 'order' => $order, 'display' => $display_type, 'ship'=>$ship, 'self' => $self);
            break;
        case 'brandn':
...
function recalculate_price() {
    ...
    $sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id, c.ru_id, c.warehouse_id, c.area_id, g.promote_price, g.promote_start_date, c.goods_number, c.goods_price as c_price, ' .
            ...
            "WHERE " . $c_sess . " c.is_gift = 0 AND c.goods_id > 0 " .
    ...
    foreach ($res AS $row) {
        ...
        if ($xiangouInfo['is_xiangou'] == 1 && $nowTime > $start_date && $nowTime < $end_date) {
            ...
            $cartGoodsNumInfo = $GLOBALS['db']->getRow($sql); //获取购物车数量
        ...
        if ($error == 1) {
            ...
            if ($rec_id > 0) {
                ...
                if ($row['extension_code'] != 'package_buy') {
                    $goods_sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET goods_price = '$goods_price'" . $sess . $set .
                            " WHERE goods_id = '" . $row['goods_id'] . "' AND rec_id = '" . $row['rec_id'] . "' AND warehouse_id = '" . $row['warehouse_id'] . "'";
                } else {
                    if (!empty($_SESSION['user_id'])) {
                        $sess_id = "AND session_id = '" . real_cart_mac_ip() . "' ";

                        $goods_sql = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET " . $pack_sess . $set .
    ...
    while (1)
    {
        foreach ($arr AS $row)
        {
            if ($cat == $row['cat_id'])
            {
                ...
                $cats[$index]['cat_id']   = $row['cat_id'];
                $cats[$index]['cat_name'] = $row['cat_name'];
                //$cats[$index]['child_tree'] = get_child_tree($row['cat_id']);
...
{
	$sql="select article_id,cat_id,title,author,add_time from ".$GLOBALS['ecs']->table('article')." where 1 order by add_time desc limit 0,".$num;
