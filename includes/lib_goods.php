 ...
 * @return  array
 */
function get_top10($cats = '', $presale = '', $ru_id = 0)
{
...
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $select = array())
{
    ...
    if ($row !== false)
    {
        ...
        /* 是否显示商品库存数量 */
        $row['goods_number']  = ($GLOBALS['_CFG']['use_storage'] == 1) ? $row['goods_number'] : 0;
...
function get_goods_properties($goods_id, $warehouse_id = 0, $area_id = 0, $goods_attr_id = '', $attr_type = 0) {
    ...
    foreach ($res AS $row) {
        ...
        if ($row['attr_type'] == 0) {
            ...
            $row['img_site'] = get_has_attr_info($row['attr_id'], $row['attr_value'], $row['attr_img_site'], 1);
...
function get_cat_goods_index_cat2($cat_id, $num, $warehouse_id, $area_id) {
    ...
    $sql = "SELECT cat_id, cat_name FROM " .$GLOBALS['ecs']->table("category"). " WHERE parent_id = '$cat_id' ORDER BY sort_order ASC, cat_id ASC LIMIT 10";
    $res = $GLOBALS['db']->getAll($sql);
 ...
 * @return  array
 */
function goods_info($goods_id, $warehouse_id, $area_id, $select = array())
{
    $leftJoin = '';
    $leftJoin .= " left join " .$GLOBALS['ecs']->table('warehouse_goods'). " as wg on g.goods_id = wg.goods_id and wg.region_id = '$warehouse_id' ";
    $leftJoin .= " left join " .$GLOBALS['ecs']->table('warehouse_area_goods'). " as wag on g.goods_id = wag.goods_id and wag.region_id = '$area_id' ";
