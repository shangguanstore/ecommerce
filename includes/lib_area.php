...
if($_REQUEST['act'] == 'select_regionChild'){
	...
	if($region->type == 0){
		...
		if(count($district_list) > 0){
			setcookie('district', $district_list[0]['region_id'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
		}else{
			setcookie('district', 0, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
		}
...
//echo $province_id . "<br/>" . $city_id . "<br/>" . $district_id;
