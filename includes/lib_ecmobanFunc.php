...
function get_ip_area_name($ip = '', $api = "新浪"){
    ...
    $area_name = str_replace(array('省', '市', "'"), '', $region); // by wu
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, cat_alias_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('merchants_category') .
        " WHERE cat_id = '$cat_id'");
}
...
function get_table_file_name($table = '', $name = ''){
    if($table != '' && $name != ''){
        $field = mysqli_query("Describe $table $name");
...
function get_insert_warehouse_goods($goods_id = 0, $warehouse_name = array(), $warehouse_number = array(), $warehouse_price = array(), $warehouse_promote_price = array(), $user_id = 0){
	...
	for($i=0;$i<count($warehouse_name);$i++){
		if(!empty($warehouse_name[$i])){
			...
			$parent = array('goods_id' => $goods_id, 
 ...
 * 查找商品ID (按商品编号) bylu
 */
function get_products_name2($goods_sn,$table){

    $as = '';
    if($table === 'goods'){
        $select = "goods_id";
        $whereName = "goods_sn = '" .$goods_sn. "' and is_delete = 0";
    }elseif($table === 'region_warehouse'){
        $select = "region_id";
        $whereName = "region_name = '" .$goods_sn. "'";
    }
...
function get_fine_warehouse_area_all($parent_id = 0, $goods_id = 0, $goods_attr_id = 0) {
    ...
    foreach ($res as $key => $row) {
        ...
        $sql = "select * from " . $GLOBALS['ecs']->table('warehouse_area_attr') . " where goods_id = '$goods_id' and goods_attr_id = '$goods_attr_id' and area_id = '" . $row['region_id'] . "' LIMIT 1";
...
function get_main_order_goods_info($order_id = 0, $type = 0){ //is_shipping
	...
	$select = ", (SELECT shipping_type FROM " .$GLOBALS['ecs']->table('order_info'). " AS oi WHERE oi.order_id = og.order_id LIMIT 1) AS shipping_type";
	$sql = "SELECT og.*, g.goods_weight as goodsWeight, g.is_shipping $select FROM " .$GLOBALS['ecs']->table('order_goods') ." as og, " .
    ...
    foreach ($newInfo as $key => $info) {
        ...
        do {
            ...
            if ($key == 0) {
                ...
                $service_email = $GLOBALS['_CFG']['service_email']; //邮箱
                $shop_name = $GLOBALS['_CFG']['shop_name']; //店铺名称
            } else {
                ...
                //店铺名称
                $seller_name = get_shop_name($key, 1);
            ...
            if ($GLOBALS['_CFG']['sms_order_placed'] == '1' && $sms_shop_mobile != '') {
                if (!empty($auto_sms)) {
                    ...
                    if ($GLOBALS['_CFG']['sms_type'] == 0) {
                        ...
                        $sms->send($sms_shop_mobile, sprintf($msg, $row['consignee'], $row['mobile']), '', 13, 1);
    ...
    if(!empty($rec_id)){
        if($cart_goods){
            foreach($cart_goods as $row){
                if ($type == 1) {
                    $discount = compute_discount(3, $row['rec_id']);
                    $arr['subtotal_discount'] += $discount['discount'];
                }
