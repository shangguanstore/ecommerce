...
function get_contents_section($dir = "")
{
    ...
    if (empty($cer_url) || ($cer_url != $cp_str)) {
...
function get_request_filter($get = '')
{
    if ($get) {
        $_REQUEST = $get;
    }
...
function dsc_unserialize($serial_str)
{
    ...
    $out = preg_replace_callback("!s:(\d+):\"(.*?)\";!s", function ($match) {return 's:'.strlen($match[2]).':'.$match[2];}, $serial_str);
