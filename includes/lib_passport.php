...
function register($username, $password, $email, $other = array(), $register_mode=0)
{
    ...
    //用户注册方式信息
    $user_registerMode = array('email' => $email, 'mobile_phone' => $other['mobile_phone'], 'register_mode' => $register_mode);
 ...
 * @return boolen
 */
function send_account_safe_hash($user_id, $type)
{
    /* 设置验证邮件模板所需要的内容信息 */
    $template    = get_mail_template('register_validate');
    $hash = register_hash('encode', $user_id);
    ...
    switch ($type)
    {
        ...
        case 'editmail':
            $validate_email = $GLOBALS['ecs']->url() . 'user.php?act=account_safe&type=validated_email&mail_type=editmail&hash=' . $hash;
