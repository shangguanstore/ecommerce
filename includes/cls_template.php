...
class cls_template
{
    ...
    function get_val($val)
    {
        if (strrpos($val, '[') !== false) {
            //$val = preg_replace("/\[([^\[\]]*)\]/eis", "'.'.str_replace('$','\$','\\1')", $val);  //2018/01/10
            $val = preg_replace_callback(
                "/\[([^\[\]]*)\]/is",
                function($match){return '.'.str_replace('$','\$',$match[1]);},
                $val
            );
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt') {
            ...
            $source = preg_replace_callback('/<!--\s#BeginLibraryItem\s\"\/(.*?)\"\s-->.*?<!--\s#EndLibraryItem\s-->/s',
                function ($match) {
                    //return "'{include file='.strtolower('\\1'). '}'";
