...
function get_user_default($user_id = 0)
{
    ...
    $sql = "SELECT user_name, email, mobile_phone, pay_points, user_money, credit_line, last_login, is_validated, user_picture, rank_points,user_rank, nick_name, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id' LIMIT 1";
    $row = $GLOBALS['db']->getRow($sql);
...
function get_shop_name($ru_id = 0, $type = 0){
    ...
    $sql = "SELECT concat(shoprz_brandName, shopNameSuffix) as shop_name, shoprz_brandName, shopNameSuffix, rz_shopName, is_IM FROM " .$GLOBALS['ecs']->table('merchants_shop_information'). " WHERE user_id = '$ru_id' LIMIT 1";
...
function get_seller_shopinfo($ru_id, $select = array()){
 ...
 * status          状态：
 */
function presale_info($presale_id, $current_num = 0)
{
    /* 取得预售活动信息 */
   ...
   /* 格式化时间 */
    ...
    /* 统计信息 */
    $stat = presale_stat($presale_id, $presale['deposit']);
 ...
 * 店铺分类列表
 */
function get_category_store_list($ru_id, $is_url = 0, $level = 0){
    ...
    $sql = "SELECT cat_id, cat_name, parent_id, user_id AS ru_id, sort_order, " .
            ...
            " WHERE $where AND parent_id = 0 ORDER BY sort_order, cat_id DESC";
    ...
    if($res){
        foreach ($res as $key=>$row){
            ...
            //$goodsCat = get_goodsCat_num($row['cat_child'],$goods_ids, $ruCat);

            //$row['goods_num'] = count($goodsNums) + $goodsCat;
...
* 阿里大鱼短信配置
*/
function sms_ali($params = array(), $send_time = '') {
