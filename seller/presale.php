...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('cat_select',  presale_cat_list(0, $presale['cid'], true, 0, true, '', 1));
    //$smarty->assign('cat_list', cat_list_one(0)); remove by wu
...
function presale_list($ru_id)
{
    ...
    if ($result === false)
    {
        ...
        $where = (!empty($filter['keyword'])) ? " AND (ga.goods_name LIKE '%" . mysqli_like_quote($filter['keyword']) . "%')" : '';
