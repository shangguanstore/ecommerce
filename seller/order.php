...
$order_back_apply = get_merchants_permissions($_SESSION['seller_id'], 'order_back_apply');
$smarty->assign('order_back_apply', $order_back_apply); //退换货权限

$order_os_remove = get_merchants_permissions($_SESSION['seller_id'], 'order_os_remove');
...
if ($_REQUEST['act'] == 'order_query')
{
	...
	$tab_menu[] = array('curr' => 0, 'text'=> $_LANG['04_merge_order'], 'href' => 'order.php?act=merge');
    ...
    //$smarty->assign('action_link2', array('href' => 'order.php?act=merge', 'text' => $_LANG['04_merge_order']));
    /* 载入配送方式 */
    ...
    if ($order_finish)
    {
        ...
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '') {
            if ($GLOBALS['_CFG']['sms_type'] == 0) {
                ...
                $msg = sprintf($GLOBALS['_LANG']['order_shipped_sms'], $order['order_sn'], local_date($GLOBALS['_LANG']['sms_time_format']), $shop_name);
...
elseif ($_REQUEST['act'] == 'order_detection')
{
    ...
    $smarty->display('order_detection_list.htm');
}    
...
elseif ($_REQUEST['act'] == 'detection_query')
{
    ...
    make_json_result($smarty->fetch('order_detection_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
   ...
   /* 设置最长执行时间为5分钟 */
    ...
    if($_SESSION['is_ajax_detection'] == 1){
        ...
        if($result['order']){
            ...
            /* 记录日志 */
            order_action($result['order']['order_sn'], $result['order']['order_status'], SS_RECEIVED, $result['order']['pay_status'], '', $operator);
        }
...
elseif ($_REQUEST['act'] == 'edit_refound_amount')
{
    ...
    if($type == 1){
        $sql = "SELECT should_return FROM " .$GLOBALS['ecs']->table('order_return'). " WHERE ret_id = '$ret_d'";
        ...
        $sql = "SELECT SUM(return_shipping_fee) FROM " .$GLOBALS['ecs']->table('order_return'). " WHERE order_id = '$order_id'";
        $return_shipping_fee = $GLOBALS['db']->getOne($sql);

        $surplus_fee = $order_shipping_fee - $return_shipping_fee;

        if($refound_amount > $surplus_fee){
    ...
    $data = array(
        'should_return' => $should_return, //订单金额
...
elseif ($_REQUEST['act'] == 'merge')
{
	...
	$tab_menu[] = array('curr' => 0, 'text'=> $_LANG['02_order_list'], 'href' => 'order.php?act=list');
	$tab_menu[] = array('curr' => 0, 'text'=> $_LANG['03_order_query'], 'href' => 'order.php?act=order_query');
	$tab_menu[] = array('curr' => 1, 'text'=> $_LANG['04_merge_order'], 'href' => 'order.php?act=merge');
	$smarty->assign('tab_menu', $tab_menu);
	//页面分菜单 by wu end		
    ...
    //$smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $_LANG['02_order_list']));
    //$smarty->assign('action_link2', array('href' => 'order.php?act=order_query', 'text' => $_LANG['03_order_query']));
    ...
    elseif (isset($_POST['to_shipping'])) {
        ...
        if (!empty($invoice_no)) {
            ...
            $delivery_stock_sql = "SELECT DG.rec_id AS dg_rec_id, OG.rec_id AS og_rec_id, G.model_attr, G.model_inventory, DG.goods_id, DG.delivery_id, DG.is_real, DG.send_number AS sums, G.goods_number AS storage, G.goods_name, DG.send_number," .
				    ...
				    AND DG.delivery_id = D.delivery_id AND D.order_id = OG.order_id AND DG.goods_sn = OG.goods_sn AND DG.product_id = OG.product_id AND DG.delivery_id = '$delivery_id' GROUP BY OG.rec_id ";
            $delivery_stock_result = $GLOBALS['db']->getAll($delivery_stock_sql);
...
//        var_dump($order);die;
	...
	elseif (isset($_POST['refound'])) {
        $require_note = $_CFG['order_return_note'] == 1;
        $order = order_info($order_id);
        $refound_amount = empty($_REQUEST['refound_amount']) ? '' : $_REQUEST['refound_amount'];
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        if($order['extension_code'] == 'presale' && $order['pay_status'] == 0){
            ...
            order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_PAYED, $action_note,$_SESSION['seller_name']);
        }
    }
    /* 设为未付款 */
    ...
    elseif ('receive' == $operation)
    {
        ...
        /* 记录log */
        order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], $action_note,$_SESSION['seller_name']);
    }
     ...
     */ elseif ('refound' == $operation) {
        ...
        $return_info = return_order_info_byId($order_id);        //退换货订单
        ...
        $shippingFee = isset($_REQUEST['shipping']) ? $_REQUEST['shipping'] : ''; //退款运费金额
...
//        update_order($order_id, $arr);
        ...
        if ($order['pay_status'] != PS_UNPAYED) {
            ...
            $refund_type = $_REQUEST['refund'];
            $refund_amount = $_REQUEST['refound_amount'] + $_REQUEST['shipping'];

            $refund_note = $_REQUEST['refund_note'];
            //退款
			if(!empty($_REQUEST['action_note'])){
				
                                    order_refund1($order, $refund_type, $refund_amount, $refund_note);
				//标记order_return 表
...
elseif ($_REQUEST['act'] == 'json') 
{
    ...
    if ($func == 'get_goods_info')
    {
        ...
        $sql = "SELECT g.goods_id, c.cat_name, g.goods_sn, g.goods_name, b.brand_name, g.market_price, g.model_attr, g.user_id, " .
                ...
                'IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) AS shop_price, ' .
	...
	//ecmoban模板堂 --zhuo end
        ...
        $sql = "SELECT a.attr_id, a.attr_name, g.goods_attr_id, g.attr_value, " . 
                ...
                "LEFT JOIN" . $ecs->table('attribute') . " AS a ON g.attr_id = a.attr_id " .
                $attr_leftJoin . 
                "WHERE g.goods_id = '$goods_id' ORDER BY g.attr_id ASC";
	...
	if(isset($_REQUEST['attr']) && !empty($_REQUEST['attr'])){
        ...
        $sql = "SELECT a.attr_id, a.attr_name, g.goods_attr_id, g.attr_value, " . 
                ...
                "LEFT JOIN" . $ecs->table('attribute') . " AS a ON g.attr_id = a.attr_id " .
                $attr_leftJoin . 
                "WHERE g.goods_id = '$goods_id' " .$goodsAttr. " ORDER BY g.attr_id ASC";
        $goods['attr_list'] = array();
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT user_id, user_name FROM " . $GLOBALS['ecs']->table('users') .
                " WHERE user_id LIKE '%" . mysqli_like_quote($id_name) . "%'" .
                " OR user_name LIKE '%" . mysqli_like_quote($id_name) . "%'" .
                " LIMIT 20";
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
        $sql = "SELECT goods_id, goods_name, goods_sn, user_id FROM " . $GLOBALS['ecs']->table('goods') .
                ...
                $where .
                " AND is_alone_sale = 1" .
                " AND (goods_id LIKE '%" . mysqli_like_quote($keyword) . "%'" .
                " OR goods_name LIKE '%" . mysqli_like_quote($keyword) . "%'" .
                " OR goods_sn LIKE '%" . mysqli_like_quote($keyword) . "%')" .
                " LIMIT 20";
        $res = $GLOBALS['db']->query($sql);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($filter['source'] == 'start' || $adminru['ru_id'] > 0)
        {
        ...
        if(!empty($filter['start_take_time']) || !empty($filter['end_take_time']))
        {
            if ($filter['user_name'])
            {
                $sql = "SELECT o.order_id FROM " . $GLOBALS['ecs']->table('order_info') . " AS o,".
                        $GLOBALS['ecs']->table('order_action'). " as oa, " .
                        $leftJoin.
                        $GLOBALS['ecs']->table('users') . " AS u " . $where ." AND o.order_id = oa.order_id " .$where_store. $no_main_order;
            }
            else
            {
                $sql = "SELECT o.order_id FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
        ...
        }else
        {
            if ($filter['user_name'])
            {
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " AS o,".
                        $leftJoin.
                       $GLOBALS['ecs']->table('users') . " AS u " . $where .$where_store. $no_main_order;
            }
            else
            {
...
function get_order_goods($order)
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.model_inventory, g.model_attr AS model_attr, g.suppliers_id AS suppliers_id, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, " .
            "g.goods_thumb " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_thumb'] = (strpos($goods_thumb, 'http://') === 0) ? $goods_thumb : "../" . $goods_thumb;
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND do.order_sn LIKE '%" . mysqli_like_quote($filter['order_sn']) . "%'";
        }
        if ($filter['consignee'])
        {
            $where .= " AND do.consignee LIKE '%" . mysqli_like_quote($filter['consignee']) . "%'";
        }
        
        if ($filter['delivery_sn'])
        {
            $where .= " AND do.delivery_sn LIKE '%" . mysqli_like_quote($filter['delivery_sn']) . "%'";
        }
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND bo.order_sn LIKE '%" . mysqli_like_quote($filter['order_sn']) . "%'";
        }
        if ($filter['consignee'])
        {
            $where .= " AND bo.consignee LIKE '%" . mysqli_like_quote($filter['consignee']) . "%'";
        }
        if ($filter['delivery_sn'])
        {
            $where .= " AND bo.delivery_sn LIKE '%" . mysqli_like_quote($filter['delivery_sn']) . "%'";
        }
...
function download_orderlist($result) {
    ...
    $data = i('订单号,' .$data_name. '下单会员,下单时间,收货人,联系电话,地址,总金额,应付金额,确认状态,付款状态,发货状态'."\n");
    $count = count($result);
    for ($i = 0; $i < $count; $i++) {
