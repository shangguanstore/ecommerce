...
{
    if($ru_id > 0){
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('category') ." AS c, ".
                $GLOBALS['ecs']->table('merchants_category')  . " AS mc" . 
        " WHERE c.parent_id = '$parent_cat' AND c.cat_name = '$cat_name' AND c.cat_id<>'$exclude' AND c.cat_id = mc.cat_id AND mc.user_id = '$ru_id'";
...
function return_order_list() {
    ...
    if ($result === false) {
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid,".
               ...
               "r.ret_id ,r.rec_id, r.address , r.back , r.exchange ,r.attr_val , r.cause_id , r.apply_time , ".
               "r.should_return , r.actual_return , r.remark , r.address , o.sign_time ,r.return_status , r.refound_status ,  r.return_type, ".
               "r.addressee, r.phone, r.return_sn, (" . order_amount_field("o.") . ") AS total_fee, ".
