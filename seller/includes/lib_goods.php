...
//            print_r($filter['brand_id']);
	...
	$where .= $ruCat;
        ...
        $sql = "SELECT g.goods_id, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, g.is_on_sale, g.is_best, g.is_new, g.is_hot, g.sort_order, g.goods_number, g.integral, " .
            " g.is_promote, g.model_price, g.model_inventory, g.model_attr, g.review_status, g.review_content, g.store_best, g.store_new , g.store_hot , g.is_real, g.is_shipping, g.stages,g.goods_thumb, add_time,  ".
 ...
 * @return  bool                          true，重复；false，不重复
 */
function check_product_sn_exist($product_sn, $product_id = 0, $ru_id = 0, $type = 0) {
    ...
    if (empty($product_id)) {
        if ($type == 1) {
            $sql = "SELECT p.product_id FROM " . $GLOBALS['ecs']->table('products') . " AS p " . "
