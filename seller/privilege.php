...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $_POST['username'] = isset($_POST['seller_name']) ? trim($_POST['seller_name']) : '';
    $_POST['password'] = isset($_POST['password']) ? trim($_POST['password']) : '';

    $sql="SELECT `ec_salt` FROM ". $ecs->table('admin_user') ."WHERE user_name = '" . $_POST['seller_name']."'";
    ...
    if ($row)
    {
        ...
        $_SESSION['admin_ru_id'] = $row['ru_id'];
   ...
   if($ru_id)
   {
       //商家名称
        ...
        {    
            ...
            {       
                $sql = " SELECT contactPhone FROM ". $ecs->table('merchants_steps_fields') ." WHERE user_id = '$ru_id'";
                $contactPhone = $db->getOne($sql);
                $templates_sms = '';
                if($admin_name && $_POST['new_password'])
                {
                    $templates_sms = "名称：". htmlspecialchars($admin_name) ."，密码：".htmlspecialchars($_POST['new_password']);
                }else
                {
                    $templates_sms = "名称：". htmlspecialchars($admin_name);
                }

                if ($GLOBALS['_CFG']['sms_seller_signin'] == '1' && $shopinfo['mobile'] != '' && $shop_name != '')
                {
                    include_once(ROOT_PATH.'includes/cls_sms.php');
                    $sms = new sms();
                    $send_result = $sms->send($shopinfo['mobile'],
        ...
        elseif($GLOBALS['_CFG']['sms_type'] == 1)
        {
            ...
            //阿里大鱼短信接口
            $result = get_seller_edit_info_lang($str_centent);
...
function get_admin_userlist($ru_id)
{
    ...
    if ($result === false) {
        ...
        if ($filter['keywords'])
        {
            $where .= " AND (user_name LIKE '%" . mysqli_like_quote($filter['keywords']) . "%')";
        }
