...
elseif ($_REQUEST['act'] == 'upload')
{
	...
	if($_FILES['file']['name']){
		...
		if(count($data) > 0){
			...
			$return = get_insert_bacth_area($goods_list);
...
elseif ($_REQUEST['act'] == 'download')
{
    ...
    {
        $lang_file = '../languages/' . $_GET['charset'] . '/admin/goods_area_batch.php';
        ...
        if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw')
        {
			...
			if(count($area_info) > 0){
				for($i=0; $i<count($area_info); $i++){
					$data .= join(',', array($area_info[$i]['goods_name'], $area_info[$i]['area_name'], $area_info[$i]['number'], $area_info[$i]['price'], $area_info[$i]['promote_price'], $area_info[$i]['region_sort'])) . "\t\n";
				}
			}
