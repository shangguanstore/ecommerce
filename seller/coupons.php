...
if ($_REQUEST['act'] == 'list')
{
    ...
    //取出当前商家发放的优惠券
    $list = get_coupons_type_info('1,2,3,4',$_SESSION['admin_ru_id']);
...
if ($_REQUEST['act'] == 'query')
{
    ...
    if($_POST['cou_type']){
        //这里是类型搜索
        $list = get_coupons_type_info($_POST['cou_type'],$_SESSION['admin_ru_id']);
    }else{
        $list = get_coupons_type_info('1,2,3,4',$_SESSION['admin_ru_id']);
    }
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('rank_list',    get_rank_list());//等级列表 bylu
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $con_goods = empty($_POST['cou_goods'])?'0':implode(',',array_unique($_POST['cou_goods']));
    $cou_ok_goods = empty($_POST['cou_ok_goods'])?'0':implode(',',array_unique($_POST['cou_ok_goods']));

    //注册送,全场送 默认所有会员等级可参加;
    if($_POST[cou_type] == 1 || $_POST[cou_type] == 3){
    ...
    cou_name,cou_total,cou_man,cou_money,cou_user_num,cou_goods,cou_start_time,cou_end_time,cou_type,cou_get_man,
     cou_ok_user,cou_ok_goods,cou_intro,cou_add_time,ru_id,cou_title)
    VALUES (
            '$_POST[cou_name]',
            '$_POST[cou_total]',
            '$_POST[cou_man]',
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $cou_arr['cou_start_time']=local_date('Y-m-d',$cou_arr['cou_start_time']);
    $cou_arr['cou_end_time']=local_date('Y-m-d',$cou_arr['cou_end_time']);

    //可使用优惠券的商品; bylu
    if(!empty($cou_arr['cou_goods']))
        $cou_arr['goods_name']=$db->getAll("SELECT goods_id,goods_name FROM ".$ecs->table('goods')." WHERE goods_id IN(".$cou_arr['cou_goods'].")");
    //可获得优惠券的商品; bylu
    if(!empty($cou_arr['cou_ok_goods']))
        $cou_arr['cou_ok_goods_name']=$db->getAll("SELECT goods_id,goods_name FROM ".$ecs->table('goods')." WHERE goods_id IN(".$cou_arr['cou_ok_goods'].")");

    //允许领取优惠券的会员级别; bylu
    if(!empty($cou_arr['cou_ok_user'])){
        $cou_arr['cou_ok_user_name']=explode(',',$db->getOne("SELECT GROUP_CONCAT(rank_id) FROM ".$ecs->table('user_rank')." WHERE rank_id IN(".$cou_arr['cou_ok_user'].")"));
        //等级列表 bylu
        $rank_list=get_rank_list();
        foreach($rank_list as $k=>$v){
            $checked = in_array($k,$cou_arr['cou_ok_user_name'])? "checked": '';
            $rank_list[$k]=array('rank_name'=>$v,'checked'=>$checked);
        }
    }else{
        $rank_list=get_rank_list();
        foreach($rank_list as $k=>$v){
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $cou_user_num = empty($_POST['cou_user_num'])?1:$_POST['cou_user_num'];
    $cou_goods = empty($_POST['cou_goods'])?'0':implode(',',array_unique($_POST['cou_goods']));
    $cou_ok_goods = empty($_POST['cou_ok_goods'])?'0':implode(',',array_unique($_POST['cou_ok_goods']));
    ...
    /* 修改入库。 */
    $sql = "UPDATE " .$ecs->table('coupons'). " SET ".
