...
if ($_REQUEST['act'] == 'add')
{
	...
	$smarty->assign('menu_select',array('action' => '02_cat_and_goods', 'current' => ''));
...
elseif ($_REQUEST['act'] == 'upload')
{
	...
	if($_FILES['file']['name']){
		...
		if(count($data) > 0){
			...
			$return = get_insert_bacth_warehouse($goods_list);

			if($return == 1){
				$link[] = array('href' => 'goods_warehouse_batch.php?act=add', 'text' => $_LANG['back_warehouse_batch_list']);
				sys_msg("商品仓库更新成功!", 0, $link);
			}
			
			$link[] = array('href' => 'goods.php?act=add', 'text' => $_LANG['03_goods_edit']);
...
elseif ($_REQUEST['act'] == 'download')
{
    ...
    {
        $lang_file = '../languages/' . $_GET['charset'] . '/admin/goods_warehouse_batch.php';
        ...
        if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw')
        {
			...
			if(count($area_info) > 0){
				for($i=0; $i<count($area_info); $i++){
					$data .= join(',', array($area_info[$i]['goods_name'], $area_info[$i]['area_name'], $area_info[$i]['number'], $area_info[$i]['price'], $area_info[$i]['promote_price'])) . "\t\n";
				}
			}
