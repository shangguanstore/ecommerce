...
//服务态度评论
function showServer(obj,num,rankid,commentrank){
	$(obj).parent().removeClass();
	$(obj).parent().addClass("cmtServer");
	$(obj).parent().addClass("fen"+num);
	$(rankid).html(num+".0分");
	if($(commentrank))
	{
...
function hiddenServer(obj,rankid,commentrank){
	if($(commentrank).val())
	{
		showServer(obj,$(commentrank).val(),'#Server','#commentServer');
	}
	else 
	{
		showServer(obj,1,'#Server','#commentServer');
	}
}

//发货速度评论
function showDelivery(obj,num,rankid,commentrank){
	$(obj).parent().removeClass();
	$(obj).parent().addClass("cmtDelivery");
	$(obj).parent().addClass("fen"+num);
	$(rankid).html(num+".0分");
	if($(commentrank))
	{
...
function hiddenDelivery(obj,rankid,commentrank){
	if($(commentrank).val())
	{
		showDelivery(obj,$(commentrank).val(),'#Delivery','#commentDelivery');
	}
	else 
	{
		showDelivery(obj,1,'#Delivery','#commentDelivery');
	}
}
