...
function userEdit()
{
  ...
  for (i = 7; i < frm.elements.length - 2; i++)	// 从第七项开始循环检查是否为必填项
  {
	...
	{
	  msg += '- ' + needinput.innerHTML + "<i></i>不能为空" + '\n';
	}
  }
...
function submitPwdInfo(obj)
{
	...
	if(obj_div.hasClass('formEmail')){
		if(user_name.val().length==0){
			msg.show();
			msg.find('.msg_error').html('<i></i>用户名不能为空');
			return false;
		}else if(email.val().length==0){
			msg.show();
			msg.find('.msg_error').html('<i></i>邮箱不能为空');
			return false;
		}else if(document.getElementById('captcha') && captcha.val().length==0){
			msg.show();
			msg.find('.msg_error').html('<i></i>验证码不能为空');
			return false;
		}else{
			fade = true;
		}
	...
	if(obj_div.hasClass('formPhone')){ 
		if(user_name.val().length==0){
			 ...
			 msg.show();
			 msg.find('.msg_error').html('<i></i>手机不能为空');
			 return false;
		}else if(phone.val().length>0){
			if(phone.val().length != 11){
				msg.find('.msg_error').html('<i></i>'+phone_address_empty_11);
				return false;
			}else if(!myreg.test(phone.val())){
				msg.find('.msg_error').html('<i></i>'+phone_address_empty_bzq);
				return false;
			}
		}else{
			fade = true;
	...
	if(obj_div.hasClass('formWenti')){ 
		if(user_name.val().length==0){
			...
			msg.show();
			msg.find('.msg_error').html('<i></i>请选择密码提示问题');
			return false;
		}else if(passwd_answer.val().length==0){
			msg.show();
			msg.find('.msg_error').html('<i></i>问题不能为空');
			return false;
	...
	if(document.getElementById('captcha') && captcha_verification.val()==0){
		fade = false;
		msg.show();
		msg.find('.msg_error').html('<i></i>验证码错误');
	}else{
		fade = true;
	}
...
function is_mobile_phone( phone )
{
	...
	if(!myreg.test(phone)) 
	{ 
		document.getElementById('phone_notice').innerHTML = '手机号码格式有误，请输入正确的手机号';
        var submit_disabled = true;
	} 
...
function phone_callback(result)
{
  ...
  {
    document.getElementById('phone_notice').innerHTML = "手机号已经存在,请重新输入";
    document.forms['formUser'].elements['Submit'].disabled = 'disabled';
  }
}
//ecmoban模板堂 --zhuo end
