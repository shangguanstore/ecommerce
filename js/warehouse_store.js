...
//	var district_id = document.getElementById('district_id').value;
	...
	if(document.getElementById('merchant_id')){
	...
	Ajax.call(region.getGoodsFileName(), 'id=' + goods_id + '&act=in_stock' + '&province=' + province_id + "&city=" + city_id + "&district=" + district_id + "&merchant_id=" + merchant_id + "&user_id=" + user_id + "&d_null="  + d_null, region.is_inStock, "GET", "JSON");
...
function warehouse(rId,goodsId)
{
  Ajax.call(region.getGoodsFileName(), 'act=in_warehouse&pid=' + rId + '&id=' + goodsId, warehouseResponse, 'GET', 'JSON');
}
...
function change_SizePriceResponse(res){
	if(res.err_msg){
		...
		if(res.product_number == 0){
			size_goods.innerHTML = "<b>当日无货</b>";
		}else{
			document.getElementById(res.pid).value = res.product_number;
			document.getElementById(res.pid).focus();
			changePrice(); //新增 2014-03-20
			size_goods.innerHTML = "<b>当日有货，<font style='font-size:14px; color:#003D79'>" + res.attr_value + res.size_name + "</font>存货<font style='font-size:14px; color:#E6393D'>" + res.product_number + "</font>件</b>";
		}
...
region.getRegion = function(parent, type, target, obj, user_id)
{
	...
	if(type == 2)
	{
		...
		district_type.style.background = '';
		city_li.style.background = '#FFFFFF';
		province_li.style.background = '';
	...
	else if(type == 3)
	{
		...
		district_type.style.background = '#FFFFFF';
		city_li.style.background = '';
		province_li.style.background = '';
	}
	
  Ajax.call(region.getFileName(), 'type=' + type + '&target=' + target + "&parent=" + parent + "&user_id=" + user_id , region.rgionResponse, "GET", "JSON");
}
...
region.rgionResponse = function(result, text_result)
{
	...
	if (result.regions && result.type == 2)
	  {
	    ...
	    for (i = 0; i < result.regions.length; i ++ )
	    {
	        html += '<a title="'+ result.regions[i].region_name +'" href="javascript:region.getRegion(' + result.regions[i].region_id + ', 3, district_list, ' + '\'' + result.regions[i].region_name + '\'' + ',' + result.user_id + ');">' + result.regions[i].region_name + '</a>&nbsp;&nbsp;';
	    }
	    div.innerHTML = html;
		city_list_id.style.display = 'block';
		district_list_id.style.display = 'none';
		house_list.style.display = 'none';
		city_li.innerHTML = '请选择市<i class="sc-icon-right"></i>';
	  }
	else if(result.regions && result.type == 3)
	{
	    ...
	    for (i = 0; i < result.regions.length; i ++ )
	    {
	        html += '<a title="'+ result.regions[i].region_name +'" href="javascript:region.changedDis(' + result.regions[i].region_id + ',' + result.user_id + ');">' + result.regions[i].region_name + '</a>&nbsp;&nbsp;';
	    }
	    div.innerHTML = html;
...
region.selectArea = function(int, type)
{
	...
	if(type == 1)
	{
		...
		district_list_id.style.display = 'none';
		house_list.style.display = 'block';
		province_li.style.background = '#FFFFFF';
		district_type.style.background = '';
		city_li.style.background = '';
