 ...
 */
function drp(){
    $drp_id = model()->table('drp_shop')->field('id')->where("user_id=".$_SESSION['user_id'])->one();
    if($drp_id > 0){
    ...
    elseif($_GET['drp_id'] > 0){
        ...
        if($drp_info['open'] == 1){
            ...
            $parent_id = model()->table('users')->field('parent_id')->where("user_id=".$_SESSION['user_id'])->one();
            if($parent_id){
...
function get_drp($user_id=0,$is_drp=0) {
    ...
    if(class_exists('\http\wechat\controllers\IndexController')){
        $info = model()->table('wechat_user')->field('nickname, headimgurl')->where(array('ect_uid'=>$user_id))->find();
        if(!empty($info)){
            $info['headimgurl'] = $info['headimgurl'];
            $info['nickname'] = $info['nickname'];
        }else{
            $shopuser = model()->table('users')->field('user_name')->where(array('user_id'=> $user_id))->find();
            $info['headimgurl'] =  __TPL__ . 'img/no_image.jpg' ;
            $info['nickname'] = $shopuser['user_name'];
        }
    ...
    $info['shop_img']    = $shopInfo['shop_img'] ? dirname(__ROOT__) . '/data/attached/drp_logo/'.$shopInfo['shop_img'] : '';
...
function drp_audit_status($user_id=0){
    ...
    $drp_audit_status = model()->table('drp_shop')->field("audit,status")->where(array("user_id"=>$user_id))->find();
    if($drp_audit_status){
        return $drp_audit_status;
    }else{
        return false;
    }
}
 ...
 * @return array
 */
function get_goods($catid, $warehouse_id = 0, $area_id = 0,$size = 10, $page = 1,$status=0){

    $where = "g.is_on_sale = 1 AND g.dis_commission >0 AND g.is_distribution = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ";
      if($status==0){
          $where .= " AND g.is_new = 1 AND g.cat_id " . db_create_in($catid);
      }
      if($status==1){
          $where .= " AND g.cat_id " . db_create_in($catid);
      }
      if($status==2){
          $time = gmtime();
          $where .= " AND g.promote_price > 0 AND g.promote_start_date <= '$time' AND g.promote_end_date >= '$time' AND g.cat_id " . db_create_in($catid);
      }
    $leftJoin = '';
