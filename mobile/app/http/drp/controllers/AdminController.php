...
class AdminController extends BackendController
{
    ...
    {
        parent::__construct();
        include_once(BASE_PATH . 'http/' . APP_NAME . '/helpers/function_helper.php');
    ...
    public function actionShop()
    {
        ...
        $filter['page'] = '{page}';
        $offset = $this->pageLimit(U('shop', $filter), $this->page_num);
    ...
    public function actionDrpList()
    {
        ...
        $filter['where'] = $act;
        $offset = $this->pageLimit(U('drp_list', $filter), $this->page_num);
    ...
    public function actionDrpOrderList()
    {
        ...
        $ru_id = $this->model->table('admin_user')->field('ru_id')->where($condition)->one();
        if ($ru_id) {
        ...
        if ($drp_affiliate['on'] == 1) {
            ...
            $filter['page'] = '{page}';
            $offset = $this->pageLimit(U('drporderlist', $filter), $this->page_num);
        ...
        $this->display('admin_drp_order_list');
    }
                ...
                if (empty($row['drp_is_separate'])) {
                    ...
                    for ($i = 0; $i < $num; $i++) {
                        ...
                        if (empty($up_uid) || empty($row['user_name'])) {
                            ...
                            $url = __HOST__ . U('drp/user/orderdetail',array('order_id'=>$oid));
                            pushTemplate('OPENTM201812627', $pushData, $url,$up_uid);
                            //获得佣金，发送模版消息 end
                        }
                    }
