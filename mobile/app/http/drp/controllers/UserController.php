...
class UserController extends FrontendController {
    ...
    public function __construct() {
        ...
        $this->assign('custom',  $this->custom); //原分销
        $this->assign('customs',  $this->customs); //原分销商
        ...
        if(IS_POST){
            ...
            if ($amount < $this->drp['draw_money_value']){
                ...
                if($amount <= $this->drp['shop_money']){
                    ...
                    show_message(L('drp_money_submit'), L('back_drp_center'), U('index'), 'success');
                }else{
    ...
    public function actionIndex()
    {
        ...
        $this->assign('draw_money_value', $this->drp['draw_money_value']);
        $this->assign('select_url', U('drp/index/category'));
        $this->assign('url', U('drp/shop/index',array('id'=>$this->drp['drp_id'],'u'=>$_SESSION['user_id'])));//我的微店
    ...
    public function actionUserCard() {
        if(!isset($_GET['u'])){
            $this->redirect(U('drp/user/user_card', array('u' => $_SESSION['user_id'])));
        }
        $user_id = I('request.u');
        $info = get_drp($user_id);
        $this->assign('info', $info);
        $url = 'http://'.$_SERVER['HTTP_HOST'].U('shop/index',array('id'=>$info['drp_id'], 'u'=>$user_id));
        // 路径设置
        $file = STORAGE_PATH . 'qrcode/';
        // 背景图片
        $bgImg = $file . 'drp_bg.png';
        // 头像
        $avatar = $file . 'drp_' . $this->user_id . '_avatar.png';
        // 二维码
        $qrcode = $file . 'drp_' . $this->user_id . '_qrcode.png';
        // 输出图片
        $outImg = $file . 'drp_' . $this->user_id . '.png';
        // 生成条件
        $generate = false;
        ...
        if($generate) {
            ...
            \libraries\QRcode::png($qrcodeInfo['url'], $qrcode, $errorCorrectionLevel, $matrixPointSize, 2);
            // 添加二维码水印
            $img1 = new \libraries\Image();
            $img1->open($bgImg)->water($qrcode, array(186, 320), 100)->save($outImg);
            // 生成头像
            $headimg = \libraries\Http::doGet($info['headimgurl']);
            file_put_contents($avatar, $headimg);
            $img2 = new \libraries\Image();
            $img2->open($avatar)->thumb(100, 100)->save($avatar);
            // 添加头像
            $img3 = new \libraries\Image();
            $img3->open($outImg)->water($avatar, array(100, 24), 100)->text($info['nickname'], STORAGE_PATH.'fonts/msyh.ttf',17,'#EC5151', array(280, 27))->save($outImg);
        }
        $this->assign('ewm', __ROOT__ . '../data/attached/qrcode/drp_' . $this->user_id . '.png');
  ...
  */
    ...
    private function actionGetConfig() {
        ...
        $this->wechat = $wxinfo;
        if (!empty($wxinfo)) {
            $this->weObj = new \ectouch\wechat\Wechat($wxinfo);
        }
    }
     /**
     * 店铺设置
    ...
    public function actionShopConfig() {
        ...
        if(empty($drp_info['shop_img'])){
           $info['shop_img'] = __TPL__ . 'img/user-shop.png';
            ...
            $info['shop_img'] = dirname(__ROOT__) . '/data/attached/drp_logo/' . $drp_info['shop_img'];
        }
        ...
        if (IS_POST) {
            ...
            if (empty($data['mobile'])) {
             ...
             $where['user_id'] = $_SESSION['user_id'];
             $this->model->table('drp_shop')->data($data)->where($where)->update();
             show_message(L('edit_success'), $this->custom.'中心', U('drp/user/index'),'success');
           }
