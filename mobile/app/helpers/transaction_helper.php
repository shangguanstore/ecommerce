    ...
    $res = $GLOBALS['db']->query($sql);
     ...
     foreach($res as $key=>$row){
        ...
        if ($row['order_status'] == OS_UNCONFIRMED) {
            $row['handler'] = "<a class=\"btn-default\" href=\"".U('user/order/cancel',array('order_id'=>$row['order_id'])) . "\" onclick=\"if (!confirm('" . L('confirm_cancel') . "')) return false;\">" . L('cancel') . "</a>";
        }
        else if ($row['order_status'] == OS_SPLITED) {
            /* 对配送状态的处理 */
            if ($row['shipping_status'] == SS_SHIPPED) {
                @$row['handler'] = "<a class=\"btn-default\" href=\"".U('user/order/affirmreceived',array('order_id'=>$row['order_id'])). "\" onclick=\"if (!confirm('" . L('confirm_received') . "')) return false;\">" .L('received') . "</a>";
            }
            elseif ($row['shipping_status'] == SS_RECEIVED) {
                ...
                if ($row['pay_status'] == PS_UNPAYED) {
                    @$row['handler'] = "<a class=\"btn-default\" href=\"".U('user/order/detail',array('order_id'=>$row['order_id'])). '" >' . L('pay_money') . '</a>';
                }
                else {
                    @$row['handler'] = "<a  class=\"btn-default\" href=\"".U('user/order/detail',array('order_id'=>$row['order_id'])) . '">' . L('view_order') . '</a>';
                }
            }
         ...
         if ($row['order_status'] == OS_CONFIRMED  && $row['shipping_status'] == SS_RECEIVED && $row['pay_status'] == PS_UNPAYED) {
             $row['handler_return'] =  U('user/order/goodsorder',array('order_id'=>$row['order_id']));
         }
         ...
         if( $sign_time > 0){
             ...
             if( $row['order_status'] != OS_CANCELED && $row['pay_status'] == PS_PAYED){
                 if( $day < $sign_time ){
                     $row['handler_return'] =  U('user/refound/index',array('order_id'=>$row['order_id']));
                 }else{
                     @$row['handler_return'] = '';
                 }
             }
        ...
        $row['order_status'] = $os[$row[order_status]] . ',' . $ps[$row[pay_status]] . ',' . $ss[$row['shipping_status']];
         ...
         if($row['user_order'] == OS_SPLITED &&  $row['user_shipping'] == SS_RECEIVED && $row['user_pay'] == PS_PAYED){
             ...
             if($row['sign'] > 0){
                 $sign = "&sign=" . $row['sign'];
                 $sign_url = U('user/index/comment_list');
             }
             else{
                 $sign = '';
                 $sign_url = U('user/index/comment_list');
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'status'=>          $row['status'],
                        ...
                        'handler'        => $row['handler'],
                        'order_url' => U('user/order/detail', array('order_id'=>$row['order_id'])));
    }
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if (($order['order_status'] != OS_CANCELED && $order['pay_status'] == PS_UNPAYED) || 
        ($order['pay_status'] == PS_PAYED_PART && ($order['order_status'] == OS_UNCONFIRMED || $order['order_status'] == OS_CONFIRMED)))
    {
...
function get_user_bouns_list($user_id, $type = 0, $num = 10, $start = 0)
{
    ...
    foreach($res as $row){
        ...
        if (empty($row['order_id'])) {
            ...
            $row['status'] = '<a href="'.U('user/account/detail',array('order_id'=>$row['order_id'])).'" >' . L('had_use') . '</a>';
            $row['bonus_status'] = 1;
        }
