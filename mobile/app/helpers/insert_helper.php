...
function insert_ads($arr)
{
        ...
        switch ($row['media_type']) {
            case 0: // 图片广告
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                    get_ad_path($row['ad_code'], 'data/afficheimg/') : $row['ad_code'];
                $ads[] = "<a href='" .U("affiche/index/index", array("ad_id"=>$row["ad_id"], "uri"=>urlencode($row["ad_link"]))). "' target='_blank'><img src='$src' /></a>";
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                    get_ad_path($row['ad_code'], 'data/afficheimg/') : $row['ad_code'];
            ...
            case 2: // CODE
                ...
                break;
            case 3: // TEXT
                $ads[] = "<a href='" . U('affiche/index/index', array('id'=>$row['ad_id'], 'uri'=>urlencode($row['ad_link']))) . "' target='_blank'>" . htmlspecialchars($row['ad_code']) . '</a>';
                break;
        }
    }
    ...
    $val = $GLOBALS['smarty']->display($position_style, true, false);
