    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        ...
        foreach ($res AS $k => $row) {
            /*if ($row['is_show']){
                ...
                $tree_arr[$row['cat_id']]['cat_img'] = get_image_path($row['cat_img']);
            }
            if (isset($row['cat_id']) != NULL) {
            ...
            if ($row['is_show']) {
                ...
                $three_arr[$k]['url'] = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
...
function get_child_treee($tree_id = 0)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        $child_sql = 'SELECT c.cat_id, c.cat_name, c.parent_id, c.is_show, g.goods_thumb ' .
            ' FROM ' . $GLOBALS['ecs']->table('category') . ' c' .
            ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' g ON c.cat_id = g.cat_id AND g.is_on_sale = 1 AND g.is_delete = 0' .
        ...
        foreach ($res AS $k => $row) {
            ...
            if ($row['is_show']) {
                $three_arr[$k]['id'] = $row['cat_id'];
                $three_arr[$k]['name'] = $row['cat_name'];
                $three_arr[$k]['url'] = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
                $three_arr[$k]['cat_img'] = get_image_path($row['goods_thumb']);
                $three_arr[$k]['haschild'] = 0;
            }
            if (isset($row['cat_id']) != NULL) {
                $three_arr[$k]['cat_id'] = get_child_treee($row['cat_id']);
                $three_arr[$k]['haschild'] = 1;
            }
        }
    }
...
function get_top10($cats = '', $presale)
{
    ...
    if ($presale == 'presale') {
        $where .= " AND ( SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('presale_activity') . "AS pa WHERE pa.goods_id = g.goods_id) > 0 ";
    }
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++) {
        ...
        $arr[$i]['thumb'] = get_image_path($arr[$i]['goods_thumb'], true);
...
function get_recommend_goods($type = '', $cats = '', $warehouse_id = 0, $area_id = 0, $ru_id = 0, $rec_type = 0, $presale = '')
{
    ...
    if (empty($type_goods[$type])) {
        ...
        foreach ($result AS $idx => $row) {
            ...
            $goods[$idx]['shopUrl'] = U('store/index/index', array('id' => $row['user_id']));
...
function get_promote_goods($cats = '', $warehouse_id = 0, $area_id = 0)
{
    ...
    foreach ($result AS $idx => $row) {
        ...
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['shop_price'] = price_format($row['shop_price']);
        $goods[$idx]['thumb'] = get_image_path($row['goods_thumb'], true);
        $goods[$idx]['goods_img'] = get_image_path($row['goods_img']);
        $goods[$idx]['url'] = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
    }
...
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0)
{
    ...
    if ($row !== false) {
        ...
        $row['store_url'] = U('store/index/shop_info', array('id' => $row['user_id']));
 ...
 * @return  array
 */
function goods_info($goods_id, $warehouse_id, $area_id)
{
    $leftJoin = '';
    $leftJoin .= " left join " . $GLOBALS['ecs']->table('warehouse_goods') . " as wg on g.goods_id = wg.goods_id and wg.region_id = '$warehouse_id' ";
    $leftJoin .= " left join " . $GLOBALS['ecs']->table('warehouse_area_goods') . " as wag on g.goods_id = wag.goods_id and wag.region_id = '$area_id' ";
