    ...
    if ($GLOBALS['_CFG']['mail_service'] == 0 && function_exists('mail')) {
        ...
        $config = array(
            ...
            'driverConfig' => array(
                ...
                'smtp_username' => $GLOBALS['_CFG']['smtp_user'],
                'smtp_password' => $GLOBALS['_CFG']['smtp_pass'],
                'smtp_from_to' => $GLOBALS['_CFG']['smtp_user'],
                'smtp_from_name' => $shop_name
            )
        );
 ...
 * @return      string
 */
function mysqli_like_quote($str)
{
...
function read_static_cache($cache_name)
{
    ...
    if($GLOBALS['cache']->cache == 'memcached' && C('CACHE.memcached.CACHE_TYPE') == 'Memcached'){
        ...
        $cache_file_path = CACHE_PATH . 'static_caches/' . $cache_name . '.php';
