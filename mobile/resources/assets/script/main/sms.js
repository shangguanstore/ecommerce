function register_sms(){
	...
	$.post("index.php?r=user/index/register_sms", { "mobile": mobile, "sms_code": code },function(result){
		alert(result);
	}, "json");
...
function sendSms(){
	...
	RemainTime();
	$.post("index.php?r=sms/index/send", { "mobile": mobile, "sms_code": code },function(result){
		if (result.code==2){
			alert('验证码已成功发送到您的手机');
		}else{
			if(result.msg){
