...
function showtime()
{
  ...
  {
	    Temp = (Temp != '<strong></strong>' && dateLeft <= 0) ? '<em class="color-whie">' + Temp:Temp;
    ...
    if (result.error > 0) {
        ...
        if (result.error == 2)
        {
            layer.open({
                ...
                shadeClose: false,
                yes: function() {
                    location.href = 'index.php?r=user/index/add_booking&id=' + result.goods_id + '&spec=' + result.product_spec;
                },
                no: function() {
                }
            });
        ...
        // 没选规格，弹出属性选择框
            ...
            location.href = 'index.php?r=goods/index/index&id='+result.goods_id;
        ...
        if(result.store_id>0){
            window.location.href = 'index.php?r=flow&store_id='+result.store_id+'&cart_value=' + result.cart_value;
            return ;
        }
        // 更新购物车数量
        $(".cart-num").html(result.goods_number);
        var cart_url = 'index.php?r=cart/index/index';
...
function pmwin(action, param) {
    ...
    if (action == 'open') {
        ...
        $$('pmlayer').style.top = ((clientHeight - pmheight) / 2 + scrollTop)
                + 'px';
        if (!param) {
            pmframe.location = 'index.php?c=pm&a=index';
            //pmframe.location = 'pm.php';
        } else {
            pmframe.location = 'index.php?c=pm&a=index&' + param;
    ...
    if (result.error > 0) {
        if (result.error == 2) {
            layer.open({
                ...
                shadeClose: false,
                yes: function () {
                    window.location.href = 'index.php?r=user/index/add_booking&id=' + result.package_id;
                },
                no: function () {
                }
            });
...
function in_addToCart(goodsId, parentId)
{
  ...
  goods.number   = number;
  goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
  $.ajax({ url: "index.php?r=cart/index/add_to_cart",type:"POST",data:{goods: $.toJSON(goods)}, dataType:"json", success: function(data){
          if(data.error==0){
        	  d_messages('已加入购物车', 2);
          }else{
...
var quick = 1;
  ...
  $("label.ts-1"+id).each(function (){
    ...
    $.ajax({ url: "index.php?r=cart/index/add_to_cart",type:"POST",data:{goods: $.toJSON(goods)}, dataType:"json", success: function(data){
        ...
        window.location='index.php?r=cart/index';//刷新当前页面.
    }});
}
var strr = '';
   ...
   $('body').on('click' , '.j-get-one .ect-select' , function(){
	...
	$(".flow-coupon .c-btn-submit").click(function() {
	  ...
	  coupon_price = $(this).parents(".j-f-c-s-coupon-1").find(".t-goods1 .coupon-price");
            ...
            $.get("index.php?r=flow/index/change_coupont", {cou_id: cou_id}, function(result){
                if(result.error){
                    d_messages(obj.error);
                    try
                    {
...
region.cccDdd = function(district_id, city_id, user_id, d_null){
    ...
    var province_id = document.getElementById('province_id').value;
    var url = 'index.php?r=user/index/addaddress/in_stock';
    if( city_id <= 0){
        return false;
    }
    if(d_null == 1){
   ...
   $.post('index.php?r=user/index/show_region_name',{province:province,city:city,district:district},function(obj){
