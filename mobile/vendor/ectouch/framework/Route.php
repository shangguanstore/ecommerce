...
class Route {
	...
	static public function parseUrl( $rewriteRule, $rewriteOn=false) {
        ...
        if(strpos($action_name, '_')){
            $action_name = str_replace('_', ' ', $action_name);
            $action_name = ucwords(strtolower($action_name));
	...
	static public function url($route=null, $params=array()) {
		...
		$default_controller = strtolower(Config::get('DEFAULT_CONTROLLER'));
		$default_action = Config::get('DEFAULT_ACTION');
		$route = $app;
		$route .= ($default_controller != $controller || $default_action != $action) ? '/'.$controller : '';
		$route .= ($default_action != $action) ? '/'.$action : '';
