...
class MysqlDriver implements DbInterface {
	...
	public function query($sql, array $params = array()) {
		...
		$query = mysqli_query($this->_getReadLink(),$this->getSql());
		if($query){
			$data = array();
	...
	public function execute($sql, array $params = array()) {
		...
		$query = mysqli_query($this->_getWriteLink(),$this->getSql());
		if($query){
			$affectedRows = mysqli_affected_rows( $this->_getWriteLink() );
	...
	public function getSql() {
		...
		foreach($arr as $k=>$v ){
			$sql = str_replace($k, "'" . mysqli_real_escape_string($this->sqlMeta['link'],$v) . "'", $sql);
		}
		return $sql;
	}
	...
	protected  function _connect($isMaster = true) {
		...
		if(!$link){
			throw new \Exception('connect database error :'.mysqli_error(), 500);
		}
