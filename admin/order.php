...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    foreach($goods_list AS $key=>$row)
    {
        ...
        $_goods_thumb = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $_goods_thumb = (strpos($_goods_thumb, 'http://') === 0) ? $_goods_thumb : $GLOBALS['ecs']->url() . $_goods_thumb;
        $row['goods_thumb'] = $_goods_thumb;
    }
    /* 是否存在实体商品 */
    $exist_real_goods = 0;
 ...
 */ elseif ($_REQUEST['act'] == 'back_cause_list') {
	...
	if($parent_id > 0){
		$result = get_cat_level($parent_id);
	}else{
    ...
    elseif (isset($_POST['to_shipping'])) {
        ...
        if (!empty($invoice_no)) {
            ...
            if (true) {
				...
				$arr['pay_status'] = ($order['pay_code'] == "cod")? $order['pay_status']:PS_PAYED; //by wu
                $arr['shipping_status'] = $shipping_status;
                update_order($order_id, $arr);
            }
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        /* 记录log */
        if($order['extension_code'] == 'presale' && $order['pay_status'] == 0){
            order_action($order['order_sn'], OS_CONFIRMED, $order['shipping_status'], PS_PAYED_PART, $action_note);
            /* 更新 pay_log */
            update_pay_log($order_id);
        }else{
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, g.goods_thumb, g.brand_id, g.user_id AS ru_id, g.goods_number AS storage, g.model_inventory, o.goods_attr " .
            "FROM " . $ecs->table('order_goods') . " AS o ".
            "LEFT JOIN " . $ecs->table('goods') . " AS g ON o.goods_id = g.goods_id " .
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($filter['source'] == 'start' || $adminru['ru_id'] > 0 || $filter['order_type'] == 2)
        {
...
//                $where_ru = " (SELECT ru_id FROM " .$GLOBALS['ecs']->table('order_goods'). " AS og WHERE og.order_id = o.order_id LIMIT 1) = 0";    
        ...
        $sql = "SELECT ifnull(bai.is_stages,0) is_stages, o.order_id, o.main_order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid, o.is_delete," .
                "o.shipping_time, o.auto_delivery_time, o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.extension_code, o.extension_id, o.is_zc_order, " .
                "(SELECT ru_id FROM " .$GLOBALS['ecs']->table('order_goods'). " AS og WHERE og.order_id = o.order_id LIMIT 1) AS ru_id, o.pay_name, o.referer, " . 
    ...
    {
		//判断是否为门店订单 by wu
		$is_store_order = $GLOBALS['db']->getOne(" SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('store_order')." WHERE order_id = '".$value['order_id']."' ");
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['status'] >= 0)
        {
            $where .= " AND do.status = '" . mysqli_like_quote($filter['status']) . "'";
        }
...
* 
*/
function get_cat_level($parent_id = 0)
{
	$res = array();
