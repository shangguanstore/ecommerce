...
if ($_REQUEST['act'] == 'add')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('goods_produts_batch.htm');
}
...
elseif ($_REQUEST['act'] == 'upload')
{
	...
	/* 显示模板 */
	assign_query_info();
	$smarty->assign('ur_here','仓属性批量上传');
	$smarty->display('goods_produts_batch_add.htm');
...
elseif ($_REQUEST['act'] == 'ajax_insert')
{
	...
	if(isset($_SESSION['goods_list']) && $_SESSION['goods_list'])
	{
		...
		//查询数据是否已经存在;
		$sql = "select product_id from " .$GLOBALS['ecs']->table('products'). " where goods_id = '" . $result['list']['goods_id'] ."'".
...
elseif ($_REQUEST['act'] == 'download')
{
    ...
    if (isset($_LANG['upload_product']))
    {
        ...
        if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw')
        {
			...
			$attr_info = get_list_download($goods_sn, '',$_attribute,count($_attribute));
