...
if(isset($_SESSION['admin_name'])){
    $admin_sql = "select user_id from " .$GLOBALS['ecs']->table('admin_user'). " where user_name = '" .$_SESSION['admin_name']. "'";
    $uid = $GLOBALS['db']->getOne($admin_sql);

    $uname = '';
    if($_SESSION['admin_id'] > 0 && $_SESSION['admin_id'] != $uid){
        $admin_sql = "select user_name from " .$GLOBALS['ecs']->table('admin_user'). " where user_id = '" .$_SESSION['admin_id']. "'";
