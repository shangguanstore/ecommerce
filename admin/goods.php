...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
    ...
    get_common_category($goods_id); //获取常见分类
...
/* 设置属性表格 */
elseif ($_REQUEST['act'] == 'set_attribute_table') {
    check_authz_json('goods_manage');
    ...
    if (count($attr_arr) > 0) {
        ...
        foreach ($attr_arr as $key => $val) {
            ...
            $attribute_array[$i]['attr_name'] = $GLOBALS['db']->getOne(" SELECT attr_name FROM " . $GLOBALS['ecs']->table('attribute') . " WHERE attr_id ='$key' ");
            $attribute_array[$i]['attr_value'] = $val;
            /* 处理属性图片 start */
            $attr_values_arr = array();
            foreach ($val as $k => $v) {
                $sql = " SELECT * FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE attr_id = '$key' AND attr_value = '$v' AND goods_id = '$goods_id' ";
                $data = $GLOBALS['db']->getRow($sql);
                if (!$data) {
                    $sql = " INSERT INTO " . $GLOBALS['ecs']->table('goods_attr') . " (goods_id, attr_id, attr_value, admin_id) " .
                            " VALUES " .
                            " ('$goods_id', '$key', '$v', '" . $_SESSION['admin_id'] . "') ";
                    $GLOBALS['db']->query($sql);
    ...
    /* 处理属性图片 start */
    $smarty->assign('attr_spec', empty($attribute_array) ? array() : $attribute_array);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    {
        if (copy(trim($_POST['goods_img_url']), ROOT_PATH . 'temp/' . basename($_POST['goods_img_url'])))
        {
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, bar_code, " .
                    ...
                    ", goods_product_tag, is_volume, is_fullcut" . $where_drp_sql . 
                ...
                "VALUES ('$goods_name', '$goods_name_style', '$goods_sn', '$bar_code', '$catgory_id', " .
                    "'$user_cat', '$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    ...
                    ", '$group_number', '$store_new', '$store_hot', '$store_best'" .
                    ", '$_POST[goods_product_tag]', '$is_volume', '$is_fullcut'" . $where_drp_val .
    ...
    if ($is_insert)
    {
        ...
        /* 处理商品图片 by wu */
        $sql = " UPDATE " . $ecs->table('goods_gallery') . " SET goods_id = '" . $goods_id . "' WHERE goods_id = 0";
        $db->query($sql);
    }
    if (!empty($_POST['goods_img_url']) && $is_img_url == 1)
    {
        ...
        $sql = " UPDATE " . $ecs->table('goods') . 
                " SET goods_thumb = '$goods_thumb', goods_img = '$goods_img', original_img = '$original_img' WHERE goods_id = '$goods_id' ";
        $db->query($sql);
        ...
        if (isset($img))
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods_gallery') . 
                    " (goods_id, img_url, thumb_url, img_original) VALUES ('$goods_id', '$gallery_img', '$gallery_thumb', '$img')";
            $db->query($sql);
        }
    ...
    $product['product_price'] = $_POST['product_price']; //货品价格
    $product['product_market_price'] = $_POST['product_market_price']; //货品市场价格
    $product['product_warn_number'] = $_POST['product_warn_number']; //警告库存
    $product['bar_code'] = $_POST['product_bar_code']; //货品条形码
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price, model_inventory, model_attr FROM " . $ecs->table('goods') . 
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        if ($product['product_id'][$key])
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table($table) . 
                    " SET product_number = '" . $product['product_number'][$key] . "',  product_price = '" . $product['product_price'][$key] . "',  product_warn_number = '" . $product['product_warn_number'][$key] . 
    ...
    if($img_id > 0){
        ...
        if($sql = true){
             if($goods_id > 0){
                ...
                $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id=''  ORDER BY img_desc ASC ";
            }
            $img_list = $db->getAll($sql);
                /* 格式化相册图片路径 */
                if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0))
