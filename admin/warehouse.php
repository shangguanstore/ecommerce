...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($region_id > 0)
    {
        ...
        /*获取顶级仓库名称*/
        $region_type = $db->getOne("SELECT MAX(region_type) FROM".$ecs->table('region_warehouse'));
        $parent_id = 0;
        for($i=0;$i<$region_type;$i++){
    ...
    if ($region_id > 0)
    {
        ...
        $action_link = array('text' => $_LANG['back_page'], 'href' => 'warehouse.php?act=list&&pid=' . $parent_id);
    }
    else
    {
        $action_link = '';
...
{	
    ...
    if($return_data)
    {
        $db->autoExecute($ecs->table('warehouse_freight'), $other, 'UPDATE', "shipping_id = '$shipping_id' AND warehouse_id = '$warehouse_id' AND user_id = '" .$adminru['ru_id']. "' AND region_id = '$regionId'");
    }
    else
    {
