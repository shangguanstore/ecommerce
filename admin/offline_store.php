...
function get_pzd_list() {
    ...
    if ($result === false)
    {
        ...
        {
            $sql = "SELECT store_id FROM".$GLOBALS['ecs']->table('store_user')."stores_user LIKE '%" . mysqli_like_quote($filter['stores_user']) . "%' AND parent_id = 0";
            $store_id = $GLOBALS['db']->getOne($sql);
            $where .= " AND id = '" . $store_id . "'";
        }
        if ($filter['stores_name'])
        {
            $where .= " AND stores_name LIKE '%" . mysqli_like_quote($filter['stores_name']) . "%'";
        }
        ...
        if($shop_name){
            $sql = "SELECT ru_id FROM".$GLOBALS['ecs']->table("seller_shopinfo")." WHERE shop_name LIKE '%" . mysqli_like_quote($shop_name) . "%'";
            $filter['shop_ru_id'] = $GLOBALS['db']->getOne($sql);
            $where .= " AND ru_id = '".$filter['shop_ru_id']."'";
        }
			...
			if($filter['date_start_time'] == '' && $filter['date_end_time'] == ''){
				...
				$start_time = local_strtotime($filter['date_start_time']);
                        ...
                        if($filter['store_name']){
                            $sql = "SELECT id FROM".$GLOBALS['ecs']->table('offline_store')." WHERE stores_name LIKE '%" . mysqli_like_quote($filter['store_name']) . "%'";
                        ...
                        if($filter['shop_name']){
                            $sql = "SELECT ru_id FROM".$GLOBALS['ecs']->table('seller_shopinfo')." WHERE shop_name LIKE '%" . mysqli_like_quote($filter['shop_name']) . "%'";
                            $seller_id = $GLOBALS['db']->getOne($sql);
                            if($seller_id > 0){
                                 $where .= " AND og.ru_id = '$seller_id'";
                            }
                        }
			/* 分页大小 */
