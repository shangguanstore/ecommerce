    ...
    elseif($adminru['ru_id']==$comment_info['ru_id'])
    {
        ...
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
               ...
               "VALUES('$_POST[comment_type]', '$_POST[id_value]','$_POST[email]', " .
                    "'$_SESSION[admin_name]','$_POST[content]','" . gmtime() . "', "
...
function get_comment_list($ru_id)
{
    ...
    //ecmoban模板堂 --zhuo start
    $sql = "select user_id from " .$GLOBALS['ecs']->table('merchants_shop_information'). " where shoprz_brandName LIKE '%" . mysqli_like_quote($filter['keywords']) . "%' OR shopNameSuffix LIKE '%" . mysqli_like_quote($filter['keywords']) . "%'";
    ...
    $where = "1";
    $where .= (!empty($filter['keywords'])) ? " AND (content LIKE '%" . mysqli_like_quote($filter['keywords']) . "%' " .$where_user. ") " : '';
	//ecmoban模板堂 --zhuo end
...
function get_single_list($ru_id)
{
	...
	$where = (!empty($filter['keywords'])) ? " AND s.order_sn LIKE '%" . mysqli_like_quote($filter['keywords']) . "%' " : '';
