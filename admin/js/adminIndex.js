$(function(){
	...
	$(".admin-logo a").on("click",function(){
		...
		$(".admincj_nav").find(".item").eq(0).show().siblings().hide();
		...
		$('.admincj_nav').find('div[id^="adminNavTabs_"]').each(function(){
			...
			$this.find('.item > .tit > a').each(function(i){
				...
				$(this).click(function(){
					...
					//url = $(this).parent().next().show().find('a:first').attr('href');
					loadUrl(url);
				});
			});
		});
    ...
    Ajax.call('index.php?is_ajax=1&act=check_order','', function(data){
        ...
        var total = parseInt(new_orders)+parseInt(new_paid);
		//有新订单才显示订单提示数目
        ...
        if(total == 0){
            $('#msg_Container .msg_content').append('<div class="no_msg">暂无消息!</div>')
        }else{
            if(new_orders>0 && new_orders<100){
                $('#msg_Container .msg_content').append('<p class="first"><a href="order.php?act=list" target="workspace" class="message" >您有新订单!</a> <span class="tiptool">'+new_orders+'</span></p>')
            }else if(new_orders>99){
				$('#msg_Container .msg_content').append('<p class="first"><a href="order.php?act=list" class="message" >您有新订单!</a><span style="float:right; margin-top:7px;"><img src="images/gduo.png"></span></p>')
			}
            if(new_paid>0 && new_paid<100){
                $('#msg_Container .msg_content').append('<p class="last"><a href="order.php?act=list" target="workspace" class="message">您有新付款的订单!</a> <span class="tiptool">'+new_paid+'</span></p>')
            }else if(new_paid>99){
				$('#msg_Container .msg_content').append('<p class="last"><a href="order.php?act=list" class="message" >您有新订单!</a><span style="float:right; margin-top:7px;"><img src="images/gduo.png"></span></p>')
			}
        }
    }, 'GET', 'JSON');
});
