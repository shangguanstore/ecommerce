...
function get_quotation_where($filter)
{
    ...
    $where .= isset($_filter->keyword) && trim($_filter->keyword) != '' ? " AND (g.goods_name LIKE '%" . mysqli_like_quote($_filter->keyword) . "%' OR g.goods_sn LIKE '%" . mysqli_like_quote($_filter->keyword) . "%' OR g.goods_id LIKE '%" . mysqli_like_quote($_filter->keyword) . "%') " : '';
    return $where;
}
